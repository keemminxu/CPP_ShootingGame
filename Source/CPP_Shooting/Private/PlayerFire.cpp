...
void UPlayerFire::YogaFire()
{
	...
	if (gameMode)
	{
		...
		// 배치시킨다.
		bullet->SetActorLocation(me->firePosition->GetComponentLocation() + FVector(0, 45, 0));
		bullet->SetActorRotation(me->firePosition->GetComponentRotation());

		// 한발 발사한다.
		gameMode->SetBulletActive(bullet2, true);
		// 배치시킨다.
		bullet2->SetActorLocation(me->firePosition->GetComponentLocation() + FVector(0, -45, 0));
